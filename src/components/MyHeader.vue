      ...
      <form class="form-inline">
        <div class="input-group">
          ...
          <div class="input-group-append h-75">
            <button><img onclick="location.href='products.html'" class="btn p-0" :src="img/header/search.png"></button>
          </div>
        </div>
      </form>
...
export default {
  beforeCreate(){
    console.log(`MyHeader组件创建前`);
  },
  created(){
    console.log(`MyHeader组件创建后`);
  },
  beforeMount(){
     console.log(`MyHeader组件挂在前`);
  },
  mounted(){
     console.log(`MyHeader组件挂在后`);
  },
  beforeUpdate(){
     console.log(`MyHeader组件更新前`);
  },
  updated(){
     console.log(`MyHeader组件更新后`);
  },
  destroyed(){
    console.log(`MyHeader组xiaohui`);
  }
}
</script>
